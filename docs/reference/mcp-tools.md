...
The Trac MCP Server provides 29 tools organized into six categories:
...
- **Sync:** 2 tools (doc_sync, doc_sync_status)
...
**Description:** Synchronize local project files with Trac wiki pages using a named sync profile. Supports bidirectional sync with conflict detection.

**Parameters:**

| Name | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| `profile` | string | **Yes** | - | Name of sync profile from config |
| `dry_run` | boolean | No | `false` | Preview changes without applying them |
| `source_root` | string | No | *(profile source)* | Override source directory (absolute path). Defaults to profile source. |

**Success Response:**
```json
{
  "type": "text",
  "text": "Sync complete: 3 files synced, 0 conflicts."
}
```
...
- **not_found:** Sync profile not found
  ```json
  {
    "type": "text",
    "text": "Error (not_found): Sync profile 'unknown' not found.\n\nAction: Available profiles: ['docs']. Check your sync configuration."
  }
  ```
...
- **validation_error:** Missing required parameter
  ...
  {
    "type": "text",
    "text": "Error (validation_error): profile is required\n\nAction: Provide the 'profile' parameter with a sync profile name."
  }
  ```
...
**Description:** Show sync state for a profile -- last sync time, number of tracked files, any unresolved conflicts.
...
{
  "type": "text",
  "text": "Sync status for 'docs'\n  Direction:   push\n  Source:      ./docs\n  Destination: ProjectDocs\n  Last sync:   2026-02-15T00:00:00\n  Tracked files: 5\n  Conflicts:     0"
}
```
